...
class ListViewPl : AppCompatActivity() {
    var idItem = " "
    var arreglo:ArrayList<IPlaneta> = arrayListOf()
    var ss1:ISistemaSolar? = null
    var idItemSeleccionado = 0
    private var adaptor1: ArrayAdapter<IPlaneta>? = null
    override fun onCreate(savedInstanceState: Bundle?) {
        var nombreSS = intent.getStringExtra("nombreSS").toString()
    ...
    fun obtenerSS1() {
        ...
        ss
            ...
            .addOnSuccessListener {
                ss1 = ISistemaSolar(
                    it.id as String?,
                    it.data?.get("nombre").toString() as String?,
                    it.data?.get("numero_planetas").toString().toInt() as Int?,
                    it.data?.get("extension").toString().toDouble() as Double?,
                    it.data?.get("estrella").toString() as String?,
                )
                Log.d("MiTag", "idItemJ:${ss1?.getId()}")
                Log.d("MiTag", "nombreSSJ:${ss1?.getNombre()}")
                Log.d("MiTag", "numlsJ:${ss1?.getNumeroPlanetas()}")
            }
    }
    fun obtenerPls(){
        val db = Firebase.firestore
    ...
    fun agregarAArreglo(planeta: QueryDocumentSnapshot){
        val nuevoPl = IPlaneta(
            ...
            planeta.data.get("nombre").toString() as String?,
            planeta.data.get("numero_lunas").toString().toInt() as Int?,
            planeta.data.get("habitado").toString().toBoolean() as Boolean?,
            planeta.data.get("diametro").toString().toDouble() as Double?,
            planeta.data.get("clasificacion").toString() as String?,
        )
        arreglo.add(nuevoPl)
    }
    fun limpiarArreglo(){
    ...
    fun actualizarSS(){
        val db = Firebase.firestore
        val ss = db.collection("sistemas_solares")
        Log.d("MiTag", "idItem:${ss1?.getId()}")
        Log.d("MiTag", "nombreSS:${ss1?.getNombre()}")
        Log.d("MiTag", "numls:${ss1?.getNumeroPlanetas()}")
        val data = hashMapOf(
